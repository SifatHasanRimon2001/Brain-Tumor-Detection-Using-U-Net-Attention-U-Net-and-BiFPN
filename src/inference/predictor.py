...
from src.config import CLASS_MAP, Config, NUM_CLASSES_CLS, NUM_CLASSES_SEG
...
class Predictor:
    ...
    def run(self) -> dict[str, float]:
        ...
        self._generate_eval_plots(
            test_metrics, cls_logits_all, cls_labels, total_dice, total_iou,
        )
        ...
        try:
            ...
            logger.warning("Could not write report (file locked): %s", e)
    ...
    def _generate_eval_plots(
        ...
        if task in ("cls", "joint") and cls_logits_all:
            ...
            labels_t = torch.tensor(cls_labels)
    ...
    def _save_mask(self, seg_out: torch.Tensor, idx: int) -> None:
